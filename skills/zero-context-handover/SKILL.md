...
- **Windows 控制台是中文输出的坟场**：英文 locale 的 Windows（含 CI runner）stdout 默认 cp1252，print 中文直接 UnicodeEncodeError。规矩：所有可能打非 ASCII 的脚本加 `sys.stdout.reconfigure(encoding="utf-8")`，CI 加 `PYTHONUTF8=1`；且补丁要验证真落盘（锚点不存在时 replace 会静默空转，别信打印，信断言）；
